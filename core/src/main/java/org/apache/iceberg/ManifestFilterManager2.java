...
abstract class ManifestFilterManager2<F extends ContentFile<F>> {
    private static final Logger LOG = LoggerFactory.getLogger(ManifestFilterManager2.class);
    ...
    protected ManifestFilterManager2(
     ...
     */
    List<ManifestFile> filterManifests(Schema tableSchema, List<ManifestFile> manifests, List<File2> fileLogs) {
                ...
                .run(
                        index -> {
                            ...
                            ManifestFile manifest =
                                    filterManifest(tableSchema, manifests.get(index), trustManifestReferences, replaced);
                            if (replaced[0]) {
    ...
    private ManifestFile filterManifest(
            Schema tableSchema, ManifestFile manifest, boolean trustManifestReferences, boolean[] replaced) {
