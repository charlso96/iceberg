...
abstract class SnapshotProducer<ThisT> implements SnapshotUpdate<ThisT> {
  ...
  public Snapshot apply2(List<File2> fileLogs) {
    refresh();
    Snapshot parentSnapshot = SnapshotUtil.latestSnapshot(base, targetBranch);
    ...
    validate(base, parentSnapshot);
    List<ManifestFile> manifests = apply2(base, parentSnapshot, fileLogs);
    OutputFile manifestList = manifestListPath();
    ...
    try (writer) {
      // keep track of the manifest lists created
      manifestLists.add(manifestList.location());
      fileLogs.add(new File2(manifestList.location(), File2.File2Type.ADD, "manifest_list"));
