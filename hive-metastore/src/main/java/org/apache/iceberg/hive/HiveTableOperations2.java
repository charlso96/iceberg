 ...
 */
public class HiveTableOperations2 extends BaseMetastoreTableOperations
        implements HiveOperationsBase {
    private static final Logger LOG = LoggerFactory.getLogger(HiveTableOperations2.class);
    ...
    RavenCatalog ravenCatalog;

    protected HiveTableOperations2(
            Configuration conf,
        ...
        this.workspaceName = conf.get("workspace_name");
        ...
        this.ravenCatalog = ravenCatalog;
    }
    ...
    protected void doRefresh() {
        ...
        TableObject tableObject = ravenCatalog.loadTable(workspaceName, database, tableName);

        String query = String.format(Locale.getDefault(),
                "SELECT file_path FROM FILELIST SNAPSHOT TableSnapshot(%d, %d) WHERE tag = 'metadata'",
        ...
        while (bufIter.valid()) {
            metadataLocation = new String(resultSet, bufIter.dataIdx(), bufIter.elemSize(), UTF_8);
            bufIter.next();
        }

        // fall back to the HMS if the metadata does not exist in Raven.
...
//            String metadataLocation = tbl.getParameters().get(METADATA_LOCATION_PROP);
            ...
            // Don't do CAS, since the transaction logic has been moved to Raven
                ...
                if (e.getMessage() != null
                        ...
                        .contains(
                                "The table has been modified. The parameter value for key '"
                                        + HiveTableOperations2.METADATA_LOCATION_PROP
                                        + "' is")) {
